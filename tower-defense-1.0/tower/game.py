...
class GameModeElimination(GameMode):
    ...
    def has_lost(self):
        # We've lost the game the number of escaped enemies exceeds
    ...
    def create_wave(intensity):
        ...
        while True:
            ...
            for _ in range(intensity):
                # Spawn one enemy up to intensity, with a delay of
                yield 1
...
class TowerGame:
    ...
    This is where track global state that transcends individual game
...
class GameEdit(GameLoop):
    ...
    def handle_collision(self):
        ...
        for enemy, projectiles in collide_mask(enemies, projectiles):
            # The enemy is already in a dying animation state; no need to do anything.
            ...
            self.mode.killed += 1
            # ... and don't forget to destroy the projectiles ...
        ...
        if event.type == pg.MOUSEBUTTONDOWN and event.button in (
            ...
            if self.sprite_manager.selected:
                if event.button == MOUSE_LEFT:
                    # If we press the LMB with selected sprites, our
                    # intent is to place them on the map. However, if
                    # the layer of the sprite in the selected sprites
                    # list is Background, then we must instead insert
                    # them into the level tile map (as the background
                    # is not really sprites in the normal sense)
